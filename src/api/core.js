 ...
 * The core functionality are sendAsset, claimQRG and doInvoke.
...
/**
 * Perform a ClaimTransaction for all available QRG based on API
 * @param {object} config - Configuration object.
...
export const claimQRG = config => {
  return fillUrl(config)
    ...
    .catch(err => {
      ...
      log.error(`claimQRG failed with ${err.message}. Dumping config`, dump)
      throw err
    })
}
 ...
 * @param {number} assetAmts.QRS - Amt of QRS to send.
 * @param {number} assetAmts.QRG - Amt of QRG to send.
...
/**
 * Helper method to get the maximum amount of gas claimable after spending all QRS.
 * @param {object} config - Configuration object.
 * @param {string} config.net - 'MainNet', 'TestNet'
 * @param {string} config.address - Wallet address
 * @param {object} api - The endpoint APi object. eg, qurasDB.
